...
import {
  Box, Card, CardContent, Typography, Button, Chip, LinearProgress, Stack,
} from "@mui/material";
import {
  ...
  return (
    <Box>
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3, lineHeight: 1.8 }}>
        ...
        comentarios, y una parte significativa contiene <strong>insultos, racismo, machismo y acoso</strong>.
        Un equipo humano simplemente <strong>no puede revisarlos todos</strong>.
      </Typography>
      <Box sx={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 2, mb: 3 }}>
        {stats.map(({ num, desc }) => (
          <Box key={num} sx={{ bgcolor: "#fff", border: "1px solid #fad2cf", borderRadius: 3, p: 2.5, textAlign: "center" }}>
      ...
      <Alert_ color={G_RED} bg="#fce8e6">
        💡 <strong>El reto:</strong> automatizar la detección para que escale junto a la plataforma,
        sin necesitar más personal. El cliente prioriza una solución <strong>práctica y desplegable</strong>.
      </Alert_>
    </Box>
  );
}
...
function SlideSolution() {
  ...
  return (
    <Box>
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3, lineHeight: 1.8 }}>
        Construimos un sistema completo de extremo a extremo: desde la recogida de datos hasta la interfaz,
        pasando por el preprocesamiento, entrenamiento y despliegue. Todo basado en <strong>NLP + ML clásico</strong>.
      </Typography>
        ...
        {steps.map((s, i) => (
          <Box key={i} sx={{ display: "flex", alignItems: "center", gap: 2,
            ...
            <Box sx={{ width: 40, height: 40, borderRadius: 2, bgcolor: "#e8f0fe",
              ...
              fontSize: "1.3rem", flexShrink: 0 }}>{s.icon}</Box>
            <Box sx={{ flex: 1 }}>
              <Typography variant="subtitle2" fontWeight={700}>{s.title}</Typography>
              <Typography variant="caption" color="text.secondary">{s.desc}</Typography>
            </Box>
            {i < steps.length - 1 && <ArrowForwardIcon sx={{ ml: "auto", color: "#dadce0", flexShrink: 0 }} />}
          </Box>
        ))}
      </Box>
    </Box>
  ...
  return (
    <Box>
      <Typography variant="body1" color="text.secondary" sx={{ mb: 2, lineHeight: 1.8 }}>
        Los modelos de ML <strong>no entienden texto crudo</strong>. Cada paso elimina ruido
        innecesario y deja solo las palabras con carga semántica real.
      </Typography>
        ...
        {pipeline.map(({ icon, step, before, after }) => (
          <Box key={step} sx={{ display: "flex", alignItems: "center", gap: 1.5,
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography variant="caption" fontWeight={700} color="text.primary" sx={{ display: "block" }}>{step}</Typography>
      ...
      <Alert_ color={G_BLUE} bg="#e8f0fe" mt>
        🔬 Usamos <strong>NLTK</strong> (lematización + stopwords) y <strong>expresiones regulares</strong> para la limpieza.
      </Alert_>
    </Box>
  );
}
...
function SlideModel() {
  return (
    <Box>
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3, lineHeight: 1.8 }}>
        El corazón del sistema son dos técnicas en un <strong>pipeline</strong>:
        vectorización del texto + clasificador binario.
      </Typography>
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mb: 3 }}>
        <Box sx={{ p: 2.5, borderRadius: 3, bgcolor: "#f3e5f5", border: "1px solid #ce93d8" }}>
          ...
          <Typography variant="body2" color="text.secondary" lineHeight={1.7} sx={{ mb: 1.5 }}>
            Convierte cada comentario en un vector de números. Las palabras
            <strong> raras pero significativas</strong> (insultos concretos) obtienen pesos altos.
          </Typography>
        ...
        <Box sx={{ p: 2.5, borderRadius: 3, bgcolor: "#e8f0fe", border: "1px solid #a8c7fa" }}>
          ...
          <Typography variant="body2" color="text.secondary" lineHeight={1.7} sx={{ mb: 1.5 }}>
            Aprende qué combinaciones de palabras son típicas de comentarios tóxicos y devuelve
            una <strong>probabilidad de toxicidad</strong> entre 0 y 1.
          </Typography>
          <Box sx={{ p: 1.2, borderRadius: 2, bgcolor: "#fff", fontFamily: "monospace",
      ...
      <Alert_ color="#9c27b0" bg="#f3e5f5">
        💡 Usamos <strong>bigramas</strong> (pares de palabras como "te odio") además de palabras sueltas.
        Esto captura frases de odio compuestas que una palabra sola no detectaría.
      </Alert_>
    </Box>
  );
}
...
function SlideResults() {
  const metrics = [
    ...
    { label: "Diferencia train / test",  val: 3,  color: "#e37400", max: 10, note: "← límite 5pp ✅" },
  ];
  return (
    <Box>
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3, lineHeight: 1.8 }}>
        El requisito más importante es controlar el <strong>overfitting</strong>. La diferencia entre
        métricas de train y test debe ser <strong>menor de 5 puntos porcentuales</strong>.
      </Typography>
      ...
      <Alert_ color={G_GREEN} bg="#e6f4ea">
        ✅ <strong>Nivel esencial superado:</strong> la diferencia train–test es de solo 3 puntos porcentuales
        (el límite es 5). El modelo generaliza bien y no memoriza los datos.
      </Alert_>
    </Box>
  );
}
...
function SlideSummary() {
  const achieved = [
    ...
    { level: "🟠 Avanzado", items: [
        "App Dockerizada (docker-compose)", "Despliegue público Render + Vercel",
      ]},
  ];
  const tech = [
...
// ── Slide definitions ─────────────────────────────────────────────────────────
const SLIDES = [
  { id: 0, tag: "El problema",   color: G_RED,    bg: "#fce8e6", icon: <YouTubeIcon    sx={{ fontSize: "2.5rem", color: G_RED }} />,    title: "YouTube tiene un problema",          subtitle: "Y se llama lenguaje de odio",                    body: SlideYoutubeProblem },
  { id: 1, tag: "La solución",   color: G_BLUE,   bg: "#e8f0fe", icon: <ShieldIcon     sx={{ fontSize: "2.5rem", color: G_BLUE }} />,   title: "Nuestra solución: IA al rescate",    subtitle: "Un detector automático de mensajes de odio",     body: SlideSolution },
  { id: 2, tag: "Preprocesado",  color: G_BLUE,   bg: "#e8f0fe", icon: <CodeIcon       sx={{ fontSize: "2.5rem", color: G_BLUE }} />,   title: "Preprocesamiento del texto",         subtitle: "Limpiar el lenguaje para que la IA lo entienda", body: SlidePreprocessing },
  { id: 3, tag: "El modelo",     color: "#9c27b0", bg: "#f3e5f5", icon: <PsychologyIcon sx={{ fontSize: "2.5rem", color: "#9c27b0" }} />, title: "El modelo de Machine Learning",     subtitle: "TF-IDF + Regresión Logística",                   body: SlideModel },
  { id: 4, tag: "Resultados",    color: G_GREEN,  bg: "#e6f4ea", icon: <BarChartIcon   sx={{ fontSize: "2.5rem", color: G_GREEN }} />,  title: "Resultados y métricas",              subtitle: "¿Qué tan bien funciona?",                        body: SlideResults },
  { id: 5, tag: "Resumen",       color: G_BLUE,   bg: "#e8f0fe", icon: <EmojiObjectsIcon sx={{ fontSize: "2.5rem", color: G_BLUE }} />, title: "Resumen del proyecto",               subtitle: "Lo que hemos construido",                        body: SlideSummary },
];
...
export default function PresentationTab() {
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          <Typography variant="caption" color="primary" fontWeight={600}>
            🎤 Presentación técnica · {SLIDES.length} diapositivas
          </Typography>
      ...
      {/* Slide card */}
      <Box sx={{ position: "relative", overflow: "hidden", minHeight: 480 }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mt: 3 }}>
        ...
        <Button
          ...
          onClick={next} disabled={current === SLIDES.length - 1} sx={{ borderRadius: 100 }}>
          Siguiente
        </Button>
      </Box>
    </Box>
