...
export default function StatsTab({ history }) {
  ...
  const lineData = history.map((r, i) => ({ n: i + 1, conf: parseInt(r.confianza), toxic: r.label }));
  ...
  function exportCSV() {
    ...
    a.download = "estadisticas_analisis.csv";
    a.click();
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
        ...
        {kpis.map((k, i) => (
          <motion.div key={i}
            ...
            <Card elevation={0} sx={{
              ...
              border: `1px solid ${k.color}30`, background: k.bg,
            }}>
            ...
            <ResponsiveContainer width="100%" height={230}>
              <PieChart>
                <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%"
                  outerRadius={88} labelLine={false} label={<PieLabel />}>
            ...
            <Box sx={{ display: "flex", gap: 2, justifyContent: "center", mt: 1 }}>
              {pieData.map(e => (
                <Box key={e.name} sx={{ display: "flex", alignItems: "center", gap: 0.8 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">{e.name} ({e.value})</Typography>
                </Box>
              ))}
            </Box>
          </CardContent>
            ...
            <LineChart data={lineData} margin={{ top: 10, right: 24, bottom: 10, left: 0 }}>
              ...
              <Line type="monotone" dataKey="conf" stroke="#1a73e8" strokeWidth={2.5}
                dot={props => {
                  ...
                  return <circle key={props.key} cx={props.cx} cy={props.cy} r={5} fill={c} stroke="#fff" strokeWidth={2} />;
                }} />
            </LineChart>
          </ResponsiveContainer>
      ...
      <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
        <Button variant="outlined" color="primary" startIcon={<FileDownloadIcon />} onClick={exportCSV}>
